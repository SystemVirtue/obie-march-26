  ...
  try {
    ...
    if (action === 'credit') {
      ...
      try {
        ...
        // Log the credit deposit (best-effort, don't block response)
        const { data: sessionRow } = await supabase.from('kiosk_sessions').select('player_id').eq('session_id', session_id).single();
        if (sessionRow?.player_id) {
          supabase.rpc('log_event', {
            p_player_id: sessionRow.player_id,
            p_event: 'credit_deposit',
            p_severity: 'info',
            p_payload: { source: source || 'unknown', amount, new_balance: updated, session_id },
          }).then(() => {}, (err: any) => console.error('log_event failed:', err));
        }
        return new Response(JSON.stringify({
          credits: updated
        }), {
