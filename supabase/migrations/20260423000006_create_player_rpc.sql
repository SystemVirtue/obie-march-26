...
CREATE OR REPLACE FUNCTION create_player(p_name TEXT, p_jukebox_slug TEXT)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    -- Create new player
    INSERT INTO players (name, status, jukebox_slug)
    VALUES (p_name, 'online', p_jukebox_slug)
    RETURNING * INTO v_new_player;
    
    RETURN json_build_object(
        'id', v_new_player.id,
        'name', v_new_player.name,
        'jukebox_slug', v_new_player.jukebox_slug,
