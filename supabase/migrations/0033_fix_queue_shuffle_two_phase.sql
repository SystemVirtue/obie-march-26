...
DECLARE v_current_queue_id UUID;
...
SELECT q.id,
  q.position INTO v_current_queue_id,
  v_current_position
FROM queue q
  JOIN player_status ps ON ps.player_id = q.player_id
...
WHERE player_id = p_player_id
  ...
  AND (
    v_current_queue_id IS NULL
    OR id != v_current_queue_id
  );
-- Nothing to shuffle (0 or 1 items).
IF v_orig_positions IS NULL
OR array_length(v_orig_positions, 1) < 2 THEN RETURN;
END IF;
...
PERFORM log_event(
  ...
  jsonb_build_object(
    'type',
    p_type,
    'now_playing_protected',
    v_current_queue_id IS NOT NULL,
...
GRANT EXECUTE ON FUNCTION queue_shuffle(UUID, TEXT) TO authenticated,
  service_role;