...
add column if not exists player_id uuid references public.players(id) on delete
set null,
  add column if not exists tab_id text,
  add column if not exists viewport_width integer,
  add column if not exists viewport_height integer,
  add column if not exists disconnected_at timestamptz;
-- Update connection_status check constraint to include STALE
-- First drop the old constraint if it exists
do $$ begin
alter table public.player_instances drop constraint if exists player_instances_connection_status_check;
exception
when others then null;
end $$;
-- Add the new constraint with STALE option
alter table public.player_instances
add constraint player_instances_connection_status_check check (
    connection_status in ('ONLINE', 'OFFLINE', 'STALE')
  );
...
create or replace function public.release_master_role(p_instance_id uuid) returns void language plpgsql security definer
...
set master_instance_id = null,
  ...
  last_updated = now()
where id = 1
  and master_instance_id = p_instance_id;
if found then
...
create or replace function public.cleanup_stale_connections() returns integer language plpgsql security definer
...
create or replace function public.request_player_refresh(p_requester text, p_player_id uuid) returns void language plpgsql security definer
set search_path = public as $$ begin perform public.log_player_event(
    'REFRESH_REQUEST',
    null,
...
set search_path = public as $$
declare v_current_master uuid;
v_master_last_seen timestamptz;
v_claimed boolean := false;
begin
select master_instance_id,
  master_last_seen into v_current_master,
  v_master_last_seen
...
or v_master_last_seen < now() - interval '20 seconds' then
update public.playback_control
...
perform public.log_player_event(
  ...
  p_instance_id,
  format(
    'Instance %s claimed master role (previous master was %s)',
    p_instance_id,
    coalesce(v_current_master::text, 'none')
  ),
  jsonb_build_object(
    'previous_master',
    v_current_master,
    'previous_master_last_seen',
    v_master_last_seen,
    'reason',
    case
...
begin -- Verify this instance is the current master
select master_instance_id = p_instance_id into v_is_current_master
from public.playback_control
where id = 1;
...
if random() < 0.1 then perform public.log_player_event(
  'HEARTBEAT',
  p_instance_id,
...
return true;
else -- This instance is no longer master - log it
perform public.log_player_event(
  'ERROR',
  p_instance_id,
...
grant execute on function public.release_master_role(uuid) to anon,
  authenticated,
  service_role;
grant execute on function public.cleanup_stale_connections() to authenticated,
  service_role;
grant execute on function public.request_player_refresh(text, uuid) to authenticated,
  service_role;
grant execute on function public.log_player_response(uuid, uuid, jsonb) to anon,
  authenticated,
  service_role;